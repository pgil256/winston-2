...
- [ ] Set up app shell: full-viewport Canvas, OrbitControls, ground plane, ambient + directional light
...
- [ ] Define bone hierarchy as a TS data structure (`src/rig/skeleton.ts`)
...
- [ ] Replace debug primitives with proper shapes (capsules for body/limbs, spheres for joints, cones for snout/ears)
...
- [ ] Implement preset definitions (`src/rig/presets.ts`) for T-pose, sit, stand, pounce, sleep, war-dance
- [ ] Add preset buttons to leva, click → 400ms lerp to target rotations
- [ ] Add manual sliders to leva (folder per bone group), wired to store
